...
namespace SimpleAuth.Data.PasswordReset
{
    public class PasswordResetData : BaseData<PasswordResetKey>, IPasswordResetData
    {
        ...
        {
            var passwordResetKey = await RunQueryFirst("select userid, resetkey, createddate from public.passwordresetkey where isdeleted = false and userid = @userId and resetkey = @resetKey order by createddate desc limit 1", new { userId, resetKey });
